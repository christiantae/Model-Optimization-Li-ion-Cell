...
for i = 1:datapoints
    dataset = i;  
    R0 = optimizableVariable('R0', [.01,.1],'Type','real'); 
    R1 = optimizableVariable('R1', [.01,.1],'Type','real'); 
    C1 = optimizableVariable('C1', [900, 3000],'Type','real'); 
    fun = @(x)ECM_fit_BO(x, i);
    results = bayesopt(fun, [R0, R1, C1], 'AcquisitionFunctionName', 'expected-improvement-per-second-plus', 'IsObjectiveDeterministic',false, 'MaxObjectiveEvaluations', 50) 
    x_val(:,i) = table2array(results.XAtMinObjective)'
    err_val(:,i) = results.MinObjective
end
...
save('OPT_Params_T23_BO_train_error', 'err_val')
save('OPT_Params_T23_BO.mat','SOC_pulse_range','R0_val_T23','R1_val_T23','C1_val_T23')
